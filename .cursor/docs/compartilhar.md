...
Permitir enviar, pelo sheet nativo do sistema (WhatsApp, SMS, e-mail, etc.), um resumo em **texto** da lista de compras, do cupom de despesa/receita ou de uma receita culinária. Se `navigator.share` existir (celular e também Chrome/Edge no desktop), abre o sheet. Se a API não existir ou falhar, copia o texto e avisa com toast.

## Escopo

- **Entra:** botão de compartilhar no detalhe da lista (`/new-resources/shopping/:id`), no drawer do cupom (`FinancialReceiptDrawer`) e no detalhe da receita (`/new-resources/recipes/:id`).
- **Entra:** texto formatado (itens da lista com quantidade/unidade e marca `[x]` / `[ ]`; cupom com loja/nome, data, pagamento, itens e totais; receita com título, descrição, ingredientes e modo de preparo).
- **Fica de fora:** fotos (cupom e receita), imagem/PDF da tela, agrupamento por categoria na lista, Capacitor/app nativo, compartilhar o livro inteiro de receitas.
...
| UI | `pages/NewResources/ShoppingListDetailView.tsx`, `components/financial-receipt/FinancialReceiptDrawer.tsx`, `pages/NewResources/RecipeDetailView.tsx` |
...
4. Desktop **sem** Web Share (ex.: Firefox): o botão copia o texto e mostra toast "Texto copiado". Desktop **com** Web Share (Chrome/Edge): abre o sheet nativo.
5. Cancelar o sheet (celular ou desktop) não mostra toast de erro.
